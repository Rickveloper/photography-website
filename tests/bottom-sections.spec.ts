...
test.describe('Bottom sections', () => {
    ...
    test('Blog grid: 3-col desktop, 1-col mobile; equal card heights', async ({ page }) => {
        await page.goto('/index.html#blog');
        const cards = page.locator('#blog a.bg-dark-700');
        await expect(cards.first()).toBeVisible();
        // Snapshot cards container
        await expect(page.locator('#blog .grid')).toHaveScreenshot('blog-grid.png', { animations: 'disabled' });
    });

    test('Contact: labels, autocomplete, focus, and responsive two-column', async ({ page }) => {
        ...
        // Snapshot
        await expect(page.locator('#contact')).toHaveScreenshot('contact-section.png', { animations: 'disabled' });
    });

    test('Footer: social links have aria-labels and balance on grid', async ({ page }) => {
        ...
        await expect(footer).toHaveScreenshot('footer.png', { animations: 'disabled' });
    });
});
